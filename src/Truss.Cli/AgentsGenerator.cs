    ...
    internal static class AgentsGenerator
    {
        ...
        private static string BuildManagedBlock(TrussManifest manifest)
        {
            ...
            block.AppendLine($"""
                ...
                - Compose or share value objects with truss g vo: -f for primitive fields, --vo for members that are value objects themselves, -a <Aggregate> to place it inside the owning aggregate's folder. Reference an existing one by naming it as the type (--vo Macros:MacroNutrients). Derived behavior on composites is written by hand, never generated.
