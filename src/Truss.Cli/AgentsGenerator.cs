    ...
    internal static class AgentsGenerator
    {
        ...
        private static string BuildManagedBlock(TrussManifest manifest)
        {
            ...
            block.AppendLine($"""
                ...
                - Extract a context into its own service: truss split <Context> (own database by default, --shared-database to keep the monolith's; routes and registrations move to the new host, handlers unchanged)
                ...
                - Install a module: truss add messaging|jobs|observability|auth|idempotency|support|docker
