...
- `MapCommand<T>` / `MapQuery<T>` minimal API extensions
- `RequestValidationException` → RFC 7807 `ProblemDetails` with every field error
- `BusinessRuleValidationException` → clean 4xx responses
...
| Jobs & queues | Background jobs with progress tracking (streaming, websockets or polling), pluggable brokers, dashboard |
