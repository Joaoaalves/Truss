...
- **Handler succeeds** → `CommitAsync` runs: domain events are dispatched, then all changes are persisted in a single save
- **Handler throws** → nothing is committed; the scoped context is discarded with the request and the exception propagates unchanged
