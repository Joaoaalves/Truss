...
- **Treating domain events as integration events.** Domain event handlers run inside the transaction and fail the command when they throw. Side effects that must survive on their own, such as e-mails or broker messages, belong to [integration events](messaging.md) published through the outbox.
