...
`-a <Owner>` places the value object inside the owning aggregate's or entity's folder and prints the lines that wire it in, phrased for the owner: aggregates take it through `Create`, entities through their constructor. Without it, the value object lands in the context's shared `ValueObjects/` folder, where several aggregates and entities can speak it. `--vo` works the same on `g ent` as on `g agg`: an entity's members become self-validating value objects too.
...
With [split services](cli.md#truss-split), the same command runs the whole constellation: the API, every service and the worker, one `dotnet watch` process each, output interleaved and prefixed with the host's name, all stopped together on Ctrl+C. Each service's URL is listed, and with observability installed, one trace follows a request across every host, so the dashboard shows the monolith calling the service as a single distributed trace.
