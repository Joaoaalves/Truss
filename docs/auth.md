...
- **JwtBearer wiring**: token validation configured from the options, with inbound claim mapping disabled so claims arrive exactly as issued (`sub`, `email`). The identity reads roles from the compact `role` claim and the name from `sub`, so `User.IsInRole` and `[Authorize(Roles = ...)]` work with the tokens as issued, and with the roles the rbac module resolves from the store.
